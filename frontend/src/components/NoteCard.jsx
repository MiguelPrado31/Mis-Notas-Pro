...
const NoteCard = ({ note, setNotes }) => {
    ...
    const handleDelete = async (e, id) => {
        e.preventDefault(); // Quitar la navegacion a la pagina 

        if(!window.confirm("Seguro que quieres eliminar esta Nota?")) return;

        try {
            await api.delete(`/notes/${id}`);
            setNotes((prev) => prev.filter((note) => note._id !== id)); // Deshacerce de las notas eliminadas
            toast.success("Nota eliminada con exito!");

        } catch (error) {
            console.log("Error en la funcion: handleDelete", error);
    ...
    return (
        ...
        border-t-4 border-solid border-[rgb(55,231,219)]'>
